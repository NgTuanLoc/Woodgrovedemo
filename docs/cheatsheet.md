...
**Admin console (with Aspire):** this setup runs Keycloak over **HTTPS on an Aspire-assigned port** — **not** `http://localhost:8080`. Open the **`keycloak` resource in the Aspire dashboard** to get its endpoint URL, and read the **auto-generated admin password** from that resource's details/env (username is `admin`). Then select realm `woodgrove`.
...
**Cause:** With this Aspire configuration Keycloak is served over **HTTPS on an Aspire-assigned random port** (e.g. `https://localhost:58074`), and **nothing listens on `8080`** — the browser just fails to connect. The admin password is also **auto-generated** by Aspire (not `admin`/`admin`).

**Fix:** Open the **`keycloak` resource in the Aspire dashboard** for the real URL, and read the generated admin password from that resource's details/env (username `admin`).
...
Admin UI:     <keycloak-url>/admin/   (user: admin, password: auto-generated by Aspire)
              ^ <keycloak-url> = HTTPS on an Aspire-assigned port — get it (and the
                admin password) from the Aspire dashboard's `keycloak` resource.
